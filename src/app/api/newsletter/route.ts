...
export async function POST(request: Request) {
  try {
    ...
    if (!apiKey) {
      console.error("Newsletter: RESEND_API_KEY is not defined in environment variables");
    ...
    const { error: userError } = await resend.emails.send({
      from: 'Ethenic Glitz <onboarding@resend.dev>',
      ...
      subject: 'Welcome to the Ethenic Glitz Archive',
      html: `
        <div style="font-family: serif; padding: 40px; color: #1a1a15; background-color: #fdfdfc; border: 1px solid #c5a059;">
          ...
          <p style="font-style: italic; font-size: 16px;">Thank you for joining the Ethenic Glitz inner circle.</p>
